...
- **Duplication (measured).** Counting method: `grep -l '^<name>() {' ` over the six scripts for each candidate helper name, at base `68fdbcc4`; a helper counts as duplicated when it is defined in two or more scripts. By this method, 25 helper functions are defined in 2 to 6 scripts each: `trim`, `lower`, `sanitize_token`, `make_case_id`, `append_summary`, `load_csv_header`, `need_cmd`, `die`/`info`/`warn`, `usage`, wait/slot loops, `show_progress`, `dict_get`, `detect_np`, `run_tee`, `has_constant_mesh`, time-directory helpers, `mark_failed`, and more. Copies have diverged: three summary column layouts (setup, mesh/flow/transport, post) and multiple lock styles exist. Every propagation fix so far was applied per file.
