...
| G8 | P2 | The documented S8 cwd rule is the orchestrator's only workspace channel; every runner accepts `-O` or `--output-dir`, but the orchestrator never forwards it | Owner/Architect decision: keep the S8 cwd-only contract, or additionally forward `-O` explicitly | The workspace binding stays implicit in the logged stage command, which slows failure triage | Design choice recorded in S8, not a defect | Design proposal: forward `-O "$BATCH_DIR"` in addition to the S8 cwd rule; needs a reviewed S8/S9 contract note | Low | Forwarding assertion in stub tests |
...
3. Single-channel workspace passing in the orchestrated path: all five runners accept `-O` or `--output-dir`, so a direct caller can select the workspace explicitly. The orchestrator uses only the documented S8 cwd channel and never forwards `-O`. The workspace binding is therefore implicit in every orchestrated invocation, and a reader of the stage command line cannot see which workspace the stage will use.
...
CURRENT
   |
   v
(1) Contract stabilization      PR #12 merged; Issue #9 admitted, PR #16 approved
                                at 317b219b and not merged; Issue #10; G3 decision
   |
   v
