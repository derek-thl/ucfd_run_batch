...
class GridContract:
    ...
    def cell_centers_xyz(self) -> np.ndarray:
        x, y, z = self.cell_center_axes_xyz()
...
class FieldSpec:
    ...
    def from_mapping(cls, data: Mapping[str, Any]) -> "FieldSpec":
        require_keys(
            data,
            ["source_name", "output_channels", "role", "kind"],
            "field spec",
        )

        source_name = str(data["source_name"]).strip()
        if not source_name:
            raise ContractError("field.source_name cannot be empty.")

        raw_channels = data["output_channels"]
        if not isinstance(raw_channels, list) or not raw_channels:
            raise ContractError(
                f"Field {source_name!r}.output_channels must be a "
                "non-empty array."
            )

        channels = tuple(str(value).strip() for value in raw_channels)
        if any(not channel for channel in channels):
            raise ContractError(
                f"Field {source_name!r} contains an empty output channel."
            )
        if len(set(channels)) != len(channels):
            raise ContractError(
                f"Field {source_name!r} contains duplicate output channels."
            )

        kind = str(data["kind"]).strip().lower()
        discrete_kinds = {"categorical", "binary_mask", "region_id"}
        supported_kinds = {
            "vector",
            "continuous_scalar",
            *discrete_kinds,
        ...
        if kind not in supported_kinds:
            raise ContractError(
                f"Field {source_name!r} has unsupported kind {kind!r}."
            )
        ...
        if expected_components <= 0:
            raise ContractError(
                f"Field {source_name!r}.expected_components must be positive."
            )
        if kind == "vector" and expected_components != 3:
            raise ContractError(
                f"Vector field {source_name!r} must have three components."
            )
        if expected_components != len(channels):
            raise ContractError(
                f"Field {source_name!r}: expected_components="
                f"{expected_components}, output_channels={channels}."
            )

        role = str(data["role"]).strip().upper()
        if role not in {"INPUT", "TARGET", "AUXILIARY"}:
            raise ContractError(
                f"Field {source_name!r} has unsupported role {role!r}."
            )

        association = data.get("preferred_source_association")
        if association is not None:
            association = str(association).strip().upper()
            if association not in {"POINT", "CELL"}:
                raise ContractError(
                    f"Field {source_name!r}: "
        ...
        interpolation = str(
            data.get(
                "interpolation",
                "nearest" if kind in discrete_kinds else "linear",
            )
        ).strip().lower()
        if interpolation not in {"linear", "nearest"}:
            raise ContractError(
                f"Field {source_name!r} has unsupported interpolation "
                f"{interpolation!r}."
            )
        if kind in discrete_kinds and interpolation != "nearest":
            raise ContractError(
                f"Discrete field {source_name!r} must use nearest "
                "interpolation."
            )
        ...
        if extrapolation not in {"ERROR", "MARK_INVALID"}:
            raise ContractError(
                f"Field {source_name!r} has unsupported extrapolation "
                f"policy {extrapolation!r}."
            )

        fill_value = float(data.get("fill_value_physical", 0.0))
        ...
        numeric_options = {
            "fill_value_physical": fill_value,
        ...
        for option, value in numeric_options.items():
            if not math.isfinite(value):
                raise ContractError(
                    f"Field {source_name!r}.{option} must be finite."
                )

        if not 0.0 <= minimum_valid_fraction <= 1.0:
            raise ContractError(
                f"Field {source_name!r}.minimum_valid_fraction must be "
                "between 0 and 1."
            )
        if kind == "vector" and value_offset != 0.0:
            raise ContractError(
                f"Vector field {source_name!r} cannot use value_offset."
            )
        ...
        return cls(
            ...
            value_offset=value_offset,
            rotate_vector=bool(
                data.get("rotate_vector", kind == "vector")
            ),
            require_volumetric_source=bool(
...
class BoundaryConditionSpec:
    """Per-case boundary override: physical value on one grid face."""

    channel: str
    face: str  # x_min | x_max | y_min | y_max | z_min | z_max
    value: float

    _FACES = ("x_min", "x_max", "y_min", "y_max", "z_min", "z_max")

    @classmethod
    def parse(cls, text: str) -> "BoundaryConditionSpec":
        parts = text.split(":")
        if len(parts) != 3:
            raise ContractError(
                f"Boundary condition {text!r} must be CHANNEL:FACE:VALUE."
            )
        channel, face, raw_value = (part.strip() for part in parts)
        if not channel:
            raise ContractError("Boundary condition channel cannot be empty.")
        face = face.lower()
        if face not in cls._FACES:
            raise ContractError(
                f"Boundary condition face {face!r} must be one of "
                f"{cls._FACES}."
            )
        try:
            ...
            raise ContractError(
                f"Boundary condition value {raw_value!r} is not a number."
            ) from exc
        if not math.isfinite(value):
            raise ContractError(
                f"Boundary condition value must be finite: {text!r}."
            )
        return cls(channel=channel, face=face, value=value)
...
class SharedCFDTransform:
    ...
    def _parse_field_specs(
        ...
        if not isinstance(raw_fields, list) or not raw_fields:
            raise ContractError(
                "contract.fields must be a non-empty array."
            )
        ...
        for index, raw_field in enumerate(raw_fields):
            if not isinstance(raw_field, Mapping):
                raise ContractError(
                    f"contract.fields[{index}] must be an object."
                )
            specs.append(FieldSpec.from_mapping(raw_field))
    ...
    def _validate_channel_contract(self) -> None:
        """Validate channel ownership, roles, producers, and normalization."""
        input_set = set(self.input_channels)
        target_set = set(self.target_channels)

        if len(input_set) != len(self.input_channels):
            raise ContractError("input_channels contains duplicate names.")
        if len(target_set) != len(self.target_channels):
            raise ContractError("target_channels contains duplicate names.")

        overlap = input_set & target_set
        if overlap:
            raise ContractError(
                "Channels cannot be both inputs and targets: "
                f"{sorted(overlap)}"
            )
        ...
        model_channels = input_set | target_set
        ...
        def register(
            ...
            *,
            ...
            for name in names:
                if name in declared:
                    raise ContractError(
                        f"Duplicate channel {name!r} declared by {owner}."
                    )
                declared.add(name)
                if spatial:
                    produced.add(name)
        ...
        for spec in self.field_specs:
            ...
            if (
                not spec.required
                and model_channels.intersection(spec.output_channels)
            ):
                raise ContractError(
                    f"Optional field {spec.source_name!r} produces required "
                    "model channels. Set required=true or remove those "
                    "channels from input_channels/target_channels."
                )
        ...
        derived_channels = tuple(
            str(name)
            for name in derived.get("output_channels", [])
        )
        ...
        unsupported = set(derived_channels) - implemented_derived
        if unsupported:
            raise ContractError(
                "Derived channels have no runtime producer: "
                f"{sorted(unsupported)}"
            )

        register(derived_channels, "derived_features")
        ...
        for index, parameter in enumerate(scenario_parameters):
            ...
            names: list[str] = []
            for channel_index, raw_name in enumerate(raw_channels):
                if (
                    ...
                    raise ContractError(
                        ...
                        f"[{channel_index}] must be a non-empty string."
                    )
        ...
        missing_inputs = sorted(input_set - produced)
        missing_targets = sorted(target_set - produced)
        if missing_inputs or missing_targets:
            raise ContractError(
                "Model channel order references unproduced channels: "
                f"missing_inputs={missing_inputs}, "
                f"missing_targets={missing_targets}"
            )

        target_fields_in_inputs = sorted(
            channel
            for channel in self.input_channels
            if field_roles.get(channel) == "TARGET"
        )
        if target_fields_in_inputs:
            raise ContractError(
                "TARGET field channels cannot be model inputs: "
                f"{target_fields_in_inputs}"
            )

        non_target_fields_in_targets = {
            channel: field_roles[channel]
            for channel in self.target_channels
            if channel in field_roles
            and field_roles[channel] != "TARGET"
        }
        if non_target_fields_in_targets:
            raise ContractError(
                "Field-backed target channels must have role TARGET: "
                f"{non_target_fields_in_targets}"
            )

        omitted_target_fields = sorted(
            channel
            for channel, role in field_roles.items()
            if role == "TARGET" and channel not in target_set
        )
        if omitted_target_fields:
            raise ContractError(
                "TARGET field channels are missing from target_channels: "
                f"{omitted_target_fields}"
            )

        global_spatial_overlap = (
            set(global_channels) & model_channels
        )
        if global_spatial_overlap:
            raise ContractError(
                "Global scenario channels cannot be spatial model channels: "
                f"{sorted(global_spatial_overlap)}"
            )
    ...
    def read_and_assemble_sources(
        ...
        """Read all supplied VTK datasets without assigning source families."""
        if not source_paths:
            raise SourceDataError("At least one source file is required.")
        ...
        for path in source_paths:
            ...
            for block_path, mesh in self._iter_leaf_blocks(
                ...
                source_points = vtk_to_numpy(
                    mapped.GetPoints().GetData()
                )
                self._replace_dataset_points(
                    mapped,
                    self.coordinate.map_points(source_points),
                )
            ...
            if leaf_count == 0:
                raise SourceDataError(
                    f"No non-empty vtkDataSet leaves found in {path}."
                )
    ...
    def inspect_and_validate_field_metadata(
        ...
        for spec in self.field_specs:
            ...
            for block in blocks:
                ...
                if (
                    len(associations) > 1
                    and spec.preferred_source_association is None
                ):
                    raise FieldMetadataError(
                        f"Field {spec.source_name!r} exists in both point "
                        f"and cell data in {block.source_path}:"
                        f"{block.block_path}. Set "
                        "preferred_source_association explicitly."
                    )
                association = (
                ...
                components = (
                    1 if array.ndim == 1 else int(array.shape[1])
                )
            ...
            volumetric_matches = [
                ...
                if match["is_volumetric"]
            ]

            if spec.required and not matches:
                raise FieldMetadataError(
                    f"Required field {spec.source_name!r} was not found "
                    "in any supplied source."
                )
            ...
            if (
                ...
                raise FieldMetadataError(
                    f"Required volumetric field {spec.source_name!r} was "
                    "found only on surface/non-volumetric blocks."
                )

            report["fields"][spec.source_name] = matches
        return report
        ...
        for spec in self.field_specs:
            ...
            if not candidates:
                if spec.required:
                    raise FieldMetadataError(
                        f"Required field {spec.source_name!r} was not found "
                        "in a compatible supplied source."
                    )
                continue
            ...
            if not sampled_values:
                if spec.required:
                    raise ResamplingError(
                        f"No supplied source could resample required field "
    ...
    def derive_geometry_and_boundary_features(
        ...
        if domain_mode == "canonical_extent":
            ...
            reference_spec = next(
                (
                    spec
                    for spec in self.field_specs
                    if reference_channel in spec.output_channels
                ),
                None,
            )
            if reference_spec is None:
                raise ContractError(
                    f"fluid_reference_channel {reference_channel!r} is not "
                    "produced by a source field."
                )
            ...
            reference_blocks = [
                ...
                if (
                    ...
                    and self._field_associations(
                        block.mesh,
                        reference_spec.source_name,
                    )
                )
            ]
            if not reference_blocks:
                raise ContractError(
                    "domain_mask_mode=source_bounds requires a volumetric "
                    f"source containing {reference_spec.source_name!r}."
                )

            bounds = [block.bounds_xyz for block in reference_blocks]
            ...
            x, y, z = self.grid.cell_center_axes_xyz()
            domain_mask = (
                (z[:, None, None] >= zmin)
                & (z[:, None, None] <= zmax)
                & (y[None, :, None] >= ymin)
        ...
        if wall_distance is not None:
            ...
            raw_wall_validity = np.asarray(
                validity[wall_distance_name],
                dtype=bool,
            )
            if raw_wall_validity.shape != shape:
                raise InvariantError(
                    f"Validity mask for {wall_distance_name!r} has shape "
                    f"{raw_wall_validity.shape}; expected {shape}."
                )

            wall_distance_valid = (
                raw_wall_validity & np.isfinite(wall_distance)
            )
    ...
    def apply_frozen_normalization(
        ...
        for name in dict.fromkeys(
            ...
            normalized[name] = self.normalizer.apply(
                name,
                channels[name],
            )
    ...
    def build_tensors(
        ...
        model_shape = self._resolve_model_shape()
        ...
        if self.input_channels:
            ...
            # Target-only transforms still return a valid [C,Z,Y,X] tensor.
            x_np = np.empty(
                (0, *model_shape),
                dtype=np.float32,
            )
        ...
        if x.ndim != 4:
            raise InvariantError(
                f"x must be [C,Z,Y,X], got {x.shape}."
            )
        if y is not None and y.ndim != 4:
            raise InvariantError(
                f"y must be [C,Z,Y,X], got {y.shape}."
            )
    ...
    def apply_boundary_conditions(
        ...
        channels: MutableMapping[str, np.ndarray],
        masks: Mapping[str, np.ndarray],
        boundary_conditions: Sequence[BoundaryConditionSpec],
    ) -> dict[str, Any]:
        """Overwrite physical face values before normalization."""
        if not boundary_conditions:
            return {}

        face_masks = self._make_grid_face_masks(
            self.grid.physical_shape_zyx
        )
        fluid_mask = np.asarray(masks["fluid_mask"], dtype=bool)
        provenance: dict[str, Any] = {}

        for index, spec in enumerate(boundary_conditions):
            if spec.channel not in channels:
                raise ContractError(
                    f"Boundary condition channel {spec.channel!r} was not "
                    f"produced. Available: {sorted(channels)}."
                )
            face_mask = face_masks[f"{spec.face}_face_mask"]
            apply_mask = face_mask & fluid_mask
            array = np.asarray(
                channels[spec.channel],
                dtype=np.float32,
            ).copy()
            array[apply_mask] = np.float32(spec.value)
            channels[spec.channel] = array
            provenance[f"bc_{index:02d}"] = {
                "channel": spec.channel,
                "face": spec.face,
                "value_physical": spec.value,
                "cells_set": int(apply_mask.sum()),
            }
        return provenance
    ...
    def transform(
        ...
        blocks = self.read_and_assemble_sources(source_paths)
        ...
        boundary_provenance = self.apply_boundary_conditions(
            channels,
            masks,
            boundary_conditions,
        )
        ...
        metadata = {
            ...
            "boundary_conditions": boundary_provenance,
            "tensor_hashes": {
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        ...
        required=True,
        help=(
            "One or more VTK-family source files. Fields are discovered "
            "according to the transform contract."
        ),
    )
    parser.add_argument(
        "--contract",
    ...
    parser.add_argument(
        ...
        action="append",
        default=[],
        metavar="CHANNEL:FACE:VALUE",
        help=(
            "Per-case physical boundary override applied before "
            "normalization, e.g. 'Ux:x_min:1.0'. Repeat for multiple "
            "channels/faces."
        ),
    )
    parser.add_argument(
...
def main() -> int:
    ...
    sample = transform.transform(
        source_paths=normalize_path_list(args.source),
        scenario=scenario,
        sample_id=args.sample_id,
        boundary_conditions=[
            BoundaryConditionSpec.parse(text)
            for text in args.boundary_condition
        ],
    )
